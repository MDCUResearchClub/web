    ...
    await Promise.all(
      data.map(async (researcher) => {
        const keywords = (
          await Promise.all(
            researcher.keywords.map((keyword) =>
              getV4(
                `keyword.keyword?filters[$and][0][title][$eq]=${encodeURIComponent(
                  keyword.title
                )}`
              )
            )
        ...
        const departmentTitle = (
          await fetchV3(`/departments/${researcher.division.department}`)
        ).title;
        const division = await getV4(
          `division.division?filters[$and][0][department][title][$eq]=${encodeURIComponent(
            departmentTitle
          )}&filters[$and][1][title][$eq]=${encodeURIComponent(
            researcher.division.title
          )}`
        );
        ...
        return postV4("researcher.researcher", {
          ...
          links: researcher.links.map((link) => ({
            title: link.title,
            url: link.url,
          })),
