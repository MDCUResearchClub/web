...
export function fetchV3(path) {
  return fetch(`${ENDPOINT_V3}${path}`, {
    headers: { authorization: TOKEN_V3 },
  }).then((res) => res.json());
}
...
export function postV4(path, data) {
  return fetch(`${ENDPOINT_V4}/content-manager/collection-types/api::${path}`, {
    method: "POST",
    headers: { Authorization: TOKEN_V4, "Content-Type": "application/json" },
...
export function publishV4(path) {
  return fetch(
    `${ENDPOINT_V4}/content-manager/collection-types/api::${path}/actions/publish`,
    {
      method: "POST",
      headers: { Authorization: TOKEN_V4 },
    }
